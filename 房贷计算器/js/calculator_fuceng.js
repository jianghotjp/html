 ...
 * @name picker
 * @desc 
 * @import core/zepto.js, core/zepto.ui.js, core/zepto.iscroll.js
 */
(function($, undefined) {
		csList = ['ajcs'],
		nsList = ['ajns'];
		interrestList = ['interrest'];
		scrollList = {
			'ajcs':null,
			'ajns':null,
			'interrest':null
		},
		interrestpicker = null;
		ajcspicker = null;
		ajnspicker = null;
	$.ui.define('interrestpicker', {
		_data:{
			bindId:null,
			interrestScope:[1, interrestNum]
		},
		//构造时，先调用这个函数
		_create:function(){
			var el = this.root();
		...
		_init:function(){
            ...
            interrestpicker = this;
			
			this.adjustSize();
			
			function oritationHandler(){
				that.adjustSize();
			}
			
			$('.'+that.id()+'-btn-set').click(function(){
				$('#lilvtext').html(that.getData());
				$('#zhlilvtext').html(that.getData());
				$('#sdgjjinterrest').val(get_num_round(lilv_array_json[selectedInterrest][curType][curNsType]*100,2));
				$('#zhgjjinterrest').val(get_num_round(lilv_array_json[selectedInterrest][2][curNsType]*100,2));
				$('#zhsdinterrest').val(get_num_round(lilv_array_json[selectedInterrest][1][curNsType]*100,2));
				that.destroy();
				interrestpicker = null;
				$.matchMedia('screen and (orientation: portrait)').removeListener(oritationHandler);
			});
			
			$('.'+that.id()+'-btn-cancel').click(function(){
				that.destroy();
				interrestpicker = null;
				$.matchMedia('screen and (orientation: portrait)').removeListener(oritationHandler);
			});
			
			$(this.getBindEl()).click(function(){
				this.blur();
				$(document).trigger('click');
				
				if(interrestpicker == null){
					$.ui.interrestpicker({bindId:this.id});
				}
			...
			$.each(interrestList, function(index, item){
				var delta = 0,
					liHeight = 30,
					curPos = 0,
					flag = false,
					containerId = that.getScrollContainerId(item),
					containerListId = that.getScrollULContainerId(item),
					getOffset = function(){
						var offset = 0;
						offset = that._data['interrestScope'][1] - that._data['interrestScope'][0];
						return offset;
					};
		
					curPos = liHeight*(selectedInterrest - that._data['interrestScope'][0]);

				
				var scroll = new iScroll($('#'+containerId), {
						onScrollStart:function(e){
								delta = $('#'+containerListId).offset().top;
								flag = true;
						},
						onScrollEnd:function(){
							...
							if(delta!=0){
								...
								if(t<=0){
									curPos = 0;
									this.scrollTo(0,curPos*-1, 200);
								}else if(t>=liHeight*getOffset()){
									curPos = liHeight*getOffset();
									this.scrollTo(0,curPos*-1, 200);
								}else{
									var curIndex = Math.ceil(t/liHeight);
									var curDelta = t%liHeight;
									if(curDelta>liHeight/2){
										curIndex++;
									}
									curPos = (curIndex - 1)*liHeight;
									this.scrollTo(0, curPos*-1, 200);
								}
							}
							...
							//得到当前选择的日期
								...
								selectedInterrest = curPos/liHeight + that._data['interrestScope'][0];
								//if(selectedYear != preSelectedYear){
									//有可能引起闰年的变化
								//	that.refreshDay();
								//}
				...
				scroll.scrollTo(0,curPos*-1, 200);
				scrollList[item] = scroll;
				
				
			});
			
			//旋转屏幕监听
			$.matchMedia('screen and (orientation: portrait)').addListener(oritationHandler);
		...
		_generateHTML:function(){
			...
			html += '<div id="'+this.getWrapperId()+'" class="ios">';
			html += ' <div class="dw-persp">';
			html += '  <div class="dwo"></div>';
			html += '  <div class="dw dwbg">';
			html += '   <div class="dw-arrw">';
			html += '    <div class="dw-arrw-i">';
			html += '     <div class="dw-arr"></div>';
			html += '    </div>';
			html += '   </div>';
			html += '   <div class="dwwr">';
			html += '    <div class="dwcc">';
			html += '     <div class="dwc dwsc dwhl">';
			html += '      <div class="dwwc dwrc">';
			...
			$.each(interrestList, function(index, item){
				...
				var container_id = that.getScrollContainerId(item);
				var container_list_id = that.getScrollULContainerId(item);
			
				html += '          <td>';
				html += '           <div class="dwwl dwrc dwwl0">';
				
				html += '            <div class="dwl">'+item+'</div>';
				html += '            <div class="dwww" tabindex="0">';
				html += '             <div id="'+container_id+'" style="height: 150px; min-width: 55px;" class="dww">';
				html += '              <div class="dw-ul">';
				html += '               <ul id="'+container_list_id+'" class="dw-bf">';
				html += '                <li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
				html += '                <li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
				
				for(var interrest=that._data['interrestScope'][0]; interrest<=that._data['interrestScope'][1]; interrest++){
					html += '<li style="height: 30px; line-height: 30px;text-align:center;" class="dw-li dw-v">'+interrestjson[interrest]+'</li>';
				}
				...
				html += '               </ul>';
				html += '              </div>';
				html += '              <div class="dwwol"></div>';
				html += '             </div>';
				html += '             <div class="dwwo"></div>';
				html += '            </div>';
				html += '            <div class="dwwol"></div>';
				html += '           </div>';
				html += '          </td>';
			...
			html += '    </div>';
			html += '    <div class="dwbc">';
			html += '     <span class="dwbw dwb-s"><span tabindex="0" class="dwb dwb-e '+that.id()+'-btn-set">设置</span></span><span class="dwbw dwb-c"><span tabindex="0" class="dwb dwb-e '+that.id()+'-btn-cancel">取消</span></span>';
			html += '    </div>';
			html += '   </div>';
			html += '  </div>';
			html += ' </div>';
		 ...
		 */
		refreshDay:function(){
			var scroll = scrollList['day'];
			var container_list_id = this.getScrollULContainerId('day');
			var dayNum = this.getDayNum();
			var html = '';
			html += '<li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
			html += '<li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
			for(var i=1; i<=dayNum; i++){
				html += '<li style="height: 30px; line-height: 30px;" class="dw-li dw-v">'+i+'日</li>';
			}
			html += '<li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
			html += '<li style="height: 30px; line-height: 30px;" class="dw-li dw-v">&nbsp;</li>';
			$('#'+container_list_id).empty().append(html);
			scroll.refresh();
		},
		...
		getScrollContainerId:function(suffix){
			return this.id()+'_'+suffix;
		},
	
		getScrollULContainerId:function(suffix){
			return this.id()+'_ul_'+suffix;
		},
		...
		getBindEl:function(){
			return $('#'+this._data['bindId']);
		},
		
		getData:function(){
				var ret = '';
				ret = interrestjson[selectedInterrest];
				return ret;
		},
	
		adjustSize:function(){
			$('.dw-persp').css('height', document.body.clientHeight+'px');
			$('.dw').css('width', '100%').css('top', ($(window).height()-224+document.body.scrollTop) + 'px');
		}
	});
	
	
	$.ui.define('ajcspicker', {
		_data:{
			bindId:null,
			csScope:[1, ajcsNum]
		},
		...
		_init:function(){
			...
			$.each(csList, function(index, item){
				var delta = 0,
					...
					getOffset = function(){
						var offset = 0;
						offset = that._data['csScope'][1] - that._data['csScope'][0];
						return offset;
					};
					curPos = liHeight*(selectedCs - that._data['csScope'][0]);
				...
				var scroll = new iScroll($('#'+containerId), {
						...
						onScrollEnd:function(){
							...
							//得到当前选择的日期
								selectedCs = curPos/liHeight + that._data['csScope'][0];
						}
					}
				);
		...
		_generateHTML:function(){
			...
			$.each(csList, function(index, item){
				...
				for(var cs=that._data['csScope'][0]; cs<=that._data['csScope'][1]; cs++){
					html += '<li style="height: 30px; line-height: 30px;text-align:center;" class="dw-li dw-v">'+ajcs[cs]+'</li>';
				}
	...
	$.ui.define('ajnspicker', {
		...
		_init:function(){
			...
			$('.'+that.id()+'-btn-set').click(function(){
				$('#selectedNs').html(that.getData());
				$('#zhSelectedNs').html(that.getData());
				if(selectedNs<=5)
				{
					curNsType = 5;
				}
			...
			$.each(nsList, function(index, item){
				var delta = 0,
					liHeight = 30,
					curPos = 0,
					flag = false,
					...
					getOffset = function(){
						var offset = 0;
						offset = that._data['nsScope'][1] - that._data['nsScope'][0];
						return offset;
					};
		
					curPos = liHeight*(selectedNs - that._data['nsScope'][0]);
				...
				var scroll = new iScroll($('#'+containerId), {
						...
						onScrollEnd:function(){
							...
							//得到当前选择的日期
								//preSelectedYear = selectedYear;
								selectedNs = curPos/liHeight + that._data['nsScope'][0];
								//if(selectedYear != preSelectedYear){
									//有可能引起闰年的变化
								//	that.refreshDay();
								//}
		...
		_generateHTML:function(){
			...
			$.each(nsList, function(index, item){
				...
				for(var ns=that._data['nsScope'][0]; ns<=that._data['nsScope'][1]; ns++){
					html += '<li style="height: 30px; line-height: 30px;text-align:center;" class="dw-li dw-v">'+ajns[ns]+'</li>';
				}
...
/*  |xGv00|ab7a5fbf9666b86aa28d84b895b38b23 */