...
define(function(require, exports, module) {
	...
	$
			.extend(
					$.fn,
					{
						...
						// attributes: space separated list of attributes to
						// retrieve and remove
			...
			.extend(
					$.validator,
					{
						...
						defaults : {
							...
							onclick : function(element) {
								...
								if (element.name in this.submitted) {
									...
									// or option elements, check parent select
									// in that case
						...
						prototype : {
							...
							init : function() {
								...
								function delegate(event) {
									...
									if (settings[eventType]
										...
										settings[eventType].call(validator,
												this[0], event);
									}
								}
								$(this.currentForm)
										...
										// "select" is provided as event.target
										// when clicking a option
										.validateDelegate(
								...
								// Add aria-required to any Static/Data/Class
								// required fields before first validation
								// Screen readers require this attribute to be
								// present before the initial submission
							...
							// return the generic message if present and no
							// method specific message is present
								...
								if (error.length) {
									...
									error = $(
											"<" + this.settings.errorElement
													+ ">").attr("id",
											elementID + "-error").addClass(
											this.settings.errorClass).html(
											message || "");

									// Maintain reference to the element to be
									// placed into the DOM
									place = error;
									if (this.settings.wrapper) {
									...
									if (this.labelContainer.length) {
										...
										this.settings.errorPlacement(place,
												$(element));
									} else {
									...
									if (error.is("label")) {
									...
									} else if (error.parents("label[for='"
											+ elementID + "']").length === 0) {
										// If the element is not a child of an
										// associated label, then it's necessary
										// to explicitly apply aria-describedby

										errorID = error.attr("id").replace(
												/(:|\.|\[|\])/g, "\\$1");
										...
										// assign to all elements in the same
										// group
							...
							errorsFor : function(element) {
								var name = this.idOrName(element), describer = $(
										element).attr("aria-describedby"), selector = "label[for='"
										+ name
										+ "'], label[for='"
										+ name
										+ "'] *";

								// aria-describedby should directly reference
								// the error element
								if (describer) {
									selector = selector + ", #"
						...
						staticRules : function(element) {
							var rules = {}, validator = $.data(element.form,
									"validator");
						...
						methods : {
							...
							email : function(value, element) {
								...
								// implementation, report a bug against the
								// above spec
								// Or use custom methods to implement your own
								// email validation
	...
	// provides delegate(type: String, delegate: Selector, handler: Callback)
	// plugin for easier event delegation
	// handler is only called when $(event.target).is(delegate), in the scope of
	// the jquery-object for event.target
